...
import { getUbicaciones, createUbicacion, deleteUbicacion, setDoctorConsultorioVirtual } from "./actions";
...
export default function SedesSheet({ doctor, onClose }: Props) {
  ...
  const [principalVirtual, setPrincipalVirtual] = useState(doctor?.consultorio_virtual ?? false);
  ...
  useEffect(() => {
    ...
    setPrincipalVirtual(doctor.consultorio_virtual ?? false);
    setLoading(true);
  ...
  async function handleTogglePrincipalVirtual(value: boolean) {
    if (!doctor) return;
    setSavingPrincipal(true);
    await setDoctorConsultorioVirtual(doctor.id, value);
    setPrincipalVirtual(value);
    ...
    <Sheet open={!!doctor} onOpenChange={(o) => !o && onClose()}>
      <SheetContent className="w-full sm:max-w-md flex flex-col gap-0 overflow-hidden">
        <SheetHeader className="pb-4 border-b">
          <SheetTitle>Consultorios y sedes</SheetTitle>
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-4">
          ...
          <div className="space-y-1.5">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Consultorio principal</p>
            <div className="flex items-center justify-between rounded-lg border px-3 py-2.5 bg-background">
              ...
              <Switch
                checked={principalVirtual}
                onCheckedChange={handleTogglePrincipalVirtual}
          ...
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Consultorios adicionales</p>
          {loading ? (
